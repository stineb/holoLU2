...
pdf("fig/fig2_PAGES.pdf", width=8, height=10 )
  ...
  rect( xlim[1], ylim[1], xlim[2], ycenter2+height/2+0.2, border = NA, col = add_alpha("darkgoldenrod2",0.2))
  ...
  mtext( "C balance (PgC)", side = 3, line = 2.5, adj = c(0.2), cex=1.2 )
  ...
  lines( c( right, right ), c( ycenter1+height/2+0.1,  mybar1[1,1]+height/2+0.1), lty=3 )
  lines( c( right, right ), c( ycenter1+height/2+0.1, ylim[1] ), lwd=1.5 )
  
  ## line at end point of total land and residual bar
  lines( c( dc_land_outarr$mean[4], dc_land_outarr$mean[4] ), c( ylim[1], mybar1[2,1]+height/2+0.1 ), lty=3 )
  ...
  text( rep(110,2), c( ycenter2, ycenter3 ), c("HYDE 3.2", "KK10"), adj=c(0,0.5), font = 1, cex = 1.2 )
  text( c(86,87), c( ycenter2, ycenter3 ), c("LULC", "LULC"), adj=c(0,0.5), font = 1 )
  text( c(20,20), c( ycenter2, ycenter3 ), c("non-peat natural sources", "non-peat natural sources"), adj=c(0,0.5), font = 1 )
  
  ## top-down and bottom up text
  text( -45, 2.8, "global C budget constraints", font=2, col = "grey60", cex = 1.5, srt=0, adj=c(0,0.5) )
  text( -43, -3.2, "process attribution", font=2, col =add_alpha("darkgoldenrod4",0.4), cex = 1.5, srt=0, adj=c(0,0.5) )

...
##--------------------------------
  ...
  pdf( "fig/plot_aluc_PAGESletter.pdf", width=sum(widths),height=sum(heights) )
    ...
    ##--------------------------------
      ...
      par( xpd=TRUE )
      mtext( "LUC emissions", side = 1, line = 1.0, adj = 0.2, cex = 0.7 )
      legend(
             "bottomleft",
             c( "HYDE 3.2", "KK10" ),
             bty="n",
...
##--------------------------------
  ...
  pdf( "fig/plot_fluc_PAGESletter.pdf", width=sum(widths),height=sum(heights) )
    ...
    ##--------------------------------
      ...
      plot( c(-10000,2000), ylim, type="n", ylab="cumulative LUC emissions (PgC)", xlab="age (ka BP)", xlim=xlim, ylim=ylim, axes=FALSE )

...
source( paste0(myhome, "utilities/cdf.write.R"))
source( paste0(myhome, "utilities/get.land.avail.R"))
source( paste0(myhome, 'utilities/regrid_landuse.R'))
source( paste0(myhome, 'utilities/area.R'))

dir <- paste0( myhome, "/data/landuse_data/kk10/")
...
filnam <- "data/KK11_30m_8k_merge.nc"
kk11 <- nc_open( filnam )
outlu <- array(NA, dim=c(kk11$dim$lon$len, kk11$dim$lat$len, 59))
time_kk11 <- ncvar_get( kk11, varid="time" )
lon <- ncvar_get( kk11, varid="lon" )
lat <- ncvar_get( kk11, varid="lat" )
...
for (idx in 1:length(yrlist)){
  ...
  outlu[,,idx+4] <- ncvar_get( kk11, varid="land_use", start=c(1,1,iyr), count=c(length(lon),length(lat),1) )  #get.var.ncdf(kk11,"land_use",start=c(1,1,iyr),count=c(1,1,1))
  # nc <- nc_open(paste(dir,"landuse_KK11_halfdeg_",yrstring[idx],".nc",sep=""))
...
filnam <- "data/fpast_STAGE2_hyde31_final_halfdeg.nc"
nc <- nc_open( filnam )
fpast <- ncvar_get( nc, varid="FPAST_FILLED" )
nc_close(nc)
...
cdf.write(
  ...
  lon,
  lat,
  paste("data/test_kk10_hydeslices.nc",sep=""),
  time=c(-10000,-9000,-8000,-7000, yrlist,2005), make.tdim=TRUE, units_time="years",
  vartype="NC_FLOAT",
  verbose=FALSE
)
...
## Create region mask
##------------------------------------------------------
nc <- nc_open( paste0(myhome, "data/masks/regmask_halfdeg.nc") )
...
for (idx in 1:length(outyrlist)){
  ...
  nc <- nc_open(paste("landuse_KK11_halfdeg_",yrstring[idx],".nc",sep=""))

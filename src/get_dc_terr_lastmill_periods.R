load('data/dc_terr_bauska_lastmill.Rdata')
# load('dc_terr_indus.Rdata')
source('src/get_statistics_df.R')
library(Hmisc)
...
pdf("fig/cbal_barplot_lastmill_holoLU2.pdf", width=8, height=6 )
par( las=1 )
ylim <- c(-100,100)
...
save( df_cum_land_uptake_bauska, dc_per_lastmill, dc_lastmill_outarr, file="data/dc_terr_bauska_lastmill_periods.Rdata" )

...
#   ylim <- c(0.05,1)
#   pdf( "fig/fsuit_sage.pdf", width=sum(widths), height=sum(heights) )
...
#   tmp <- abind( crop_hyde31, crop_hyde32, crop_kk11, crop_kk11d )
#   crop_max <- apply( tmp, c(1,2,3), FUN = max )
...
#   ylim <- c(0.15,1)
#   outtime <- c( -4000,-2000, 100, 1000, 1700, 2000 )
#   labtime <- c( "4000 BC (6 ka BP)", "2000 BC (4 ka BP)", "100 CE", "1000 CE", "1700 CE", "2000 CE")

#   pdf( "fig/perm_holoLU2.pdf", width=sum(widths), height=sum(heights) )
...
#     for (i in 1:length(outtime)){

#       par(mar=c(1,3,1,1),xaxs="i", yaxs="i",las=1)

#       itim <- which.min(abs(time - outtime[i]))

#       ## where cropland area is <0.1% show as non-permanent
#       out <- perm[,,itim]
#       itim_lu <- which.min(abs(time_lu - outtime[i]))

#       for (ilon in 1:dim(crop_max)[1]){
#         for (ilat in 1:dim(crop_max)[2]){
#           if (!is.na(out[ilon,ilat]) && !is.na(crop_max[ilon,ilat,itim_lu])){
#             if ( crop_max[ilon,ilat,itim_lu] < 0.001 ) {
#               out[ilon,ilat] <- 1
#             }
#           }
#         }
...
#       # axis(3,at=seq(0,1,1/(length(lon.labels)-1)),labels=F)
#       # axis(1,at=seq(0,1,1/(length(lon.labels)-1)),labels=lon.labels)
#       text( 0.03, 0.18, labtime[i], cex=2.0, adj=c(0,0) )
...
################
  ...
  for (i in 1:nper){
    ...
    filn <- paste( "fig/fig_LUC_maps/LUC_map_holocene_p",as.character(-periodsBP[i+1]),".pdf", sep="" )
    pdf( filn, width=sum(widths), height=sum(heights) )
    ...
    if (i==1){
      ...
      filn <- paste( "fig/fig_LUC_maps/LUC_map_holocene_p",as.character(-periodsBP[i]),".pdf", sep="" )
      pdf( filn, width=sum(widths), height=sum(heights) )
        ...
        text(0.03,0.08,paste("HYDE 3.1,",as.character(-periodsBP[i]),"yr BP"),cex=1.5,adj=c(0,0))
        ...
        text(0.03,0.08,paste("HYDE 3.2,",as.character(-periodsBP[i]),"yr BP"),cex=1.5,adj=c(0,0))
        ...
        text(0.03,0.08,paste("KK10,",as.character(-periodsBP[i]),"yr BP"),cex=1.5,adj=c(0,0))
        ...
        text(0.03,0.08,paste("KK10D,",as.character(-periodsBP[i]),"yr BP"),cex=1.5,adj=c(0,0))
...
#       lev <- c(0, 0.01, 0.1, 0.15, 0.2, 0.4, 0.7, 1)
#       color <- c("wheat3","green4","yellow","orange","red")
#       filn <- paste( "fig/fig_LUC_maps/LUC_map_lastmill_p",as.character(period_margins[i+1]),".pdf", sep="" )
#       pdf( filn, width=sum(widths), height=sum(heights) )
...
#               col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#               )
#         text(0.03,0.08,paste("HYDE 3.1,",as.character(period_margins[i+1]),"CE"),cex=1.5,adj=c(0,0))
...
#               col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#               )
#         text(0.03,0.08,paste("HYDE 3.2,",as.character(period_margins[i+1]),"CE"),cex=1.5,adj=c(0,0))
...
#               col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#               )
#         text(0.03,0.08,paste("KK10,",as.character(period_margins[i+1]),"CE"),cex=1.5,adj=c(0,0))
...
#                 col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#                 )
#           text(0.03,0.08,paste("HYDE 3.1,",as.character(period_margins[i]),"CE"),cex=1.5,adj=c(0,0))
...
#                 col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#                 )
#           text(0.03,0.08,paste("HYDE 3.2,",as.character(period_margins[i]),"CE"),cex=1.5,adj=c(0,0))
...
#                 col=out.mycolorbar$colors, breaks=out.mycolorbar$margins
#                 )
#           text(0.03,0.08,paste("KK10,",as.character(period_margins[i]),"CE"),cex=1.5,adj=c(0,0))
...
#   ylim <- c(0,1)
#   pdf( "fig/fopen_r0_holoLU2.pdf", width=sum(widths), height=sum(heights) )
...
################
  ## Read file
  nc <- open.ncdf( "data/fpast_STAGE1_hyde31_final_halfdeg.nc" )
  ...
  # tmp <- abind( past_hyde31, past_hyde32, past_kk11, past_kk11d )
  # past_max <- apply( tmp, c(1,2,3), FUN = max )

  ## field to be plotted
  itim1 <- which.min(abs(time - 1500))
  out1 <- fpast[,,itim1]
  out1[ which( past_max[,,which.min(abs(time_lu-1500))] == 0 ) ] <- 0

  itim2 <- which.min(abs(time - 1850))
  out2 <- fpast[,,itim2]
  out2[ which( past_max[,,which.min(abs(time_lu-1850))] == 0 ) ] <- 0
  ...
  ylim <- c(0.05,1)
  pdf( "fig/fpast_hyde31_halfdeg.pdf", width=sum(widths), height=sum(heights) )
    ...
    out1[is.na(out1)] <- -0.99
    out2[is.na(out2)] <- -0.99

    out1[is.na(suit_sage)] <- NA
    out2[is.na(suit_sage)] <- NA

    out1[ fpast[,,itim1]<0.0001 ] <- 0.00001
    ...
    text(0.03,0.08,"1500 CE",cex=2,adj=c(0,0))
    ...
    text(0.03,0.08,"1850 CE",cex=2,adj=c(0,0))
...
#   ylim <- c(0.05,1)
#   pdf( "fig/fpast_hyde31_halfdeg_alternativeplot.pdf", width=sum(widths), height=sum(heights) )
...
# #   # ylim <- c(0.15,0.95)
# #   ylim <- c(0,1)
# #   pdf( "fig/fcrop_EARLY_holoLU2_lpjgr.pdf", width=sum(widths), height=sum(heights) )
...
#   # # ylim <- c(0.15,0.95)
#   # ylim <- c(0,1)
#   # pdf( "fig/fcrop_LATE_holoLU2_lpjgr.pdf", width=sum(widths), height=sum(heights) )

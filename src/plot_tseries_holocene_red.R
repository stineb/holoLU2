...
off <- df_cum_pt_mean$mean[ which(df_pt_mean$year==firstyear)]
...
pdf( "fig/tseries_holocene.pdf", width=sum(widths),height=sum(heights) )
  ...
  ##--------------------------------
    ...
    polygon( 
            ...
            c( (df_cum_pt_mean$mean - df_cum_pt_mean$sd) - off, rev( (df_cum_pt_mean$mean + df_cum_pt_mean$sd) - off ) ), 
    ...
    # ## Add mean to plot
    lines( df_cum_pt_mean$year, df_cum_pt_mean$mean - off, col="dodgerblue4", lwd=2, lty=1 )
    # lines( df_cum_pt_mean$year, cumsum( df_pt_mean$mean ), col="red", lwd=3, lty=1  )

    ## Add Charly's mean to plot
    # lines( df_charly_pt_mean$year, df_charly_pt_mean$cumsum_mean, col="blue", lwd=1 )
  ...
  ##--------------------------------
    ...
    polygon( 
            ...
            c( (df_cum_pt_mean_lhnfix$mean - df_cum_pt_mean_lhnfix$sd) - off, rev( (df_cum_pt_mean_lhnfix$mean + df_cum_pt_mean_lhnfix$sd) - off ) ), 
    ...
    # ## Add mean to plot
    lines( df_cum_pt_mean_lhnfix$year, df_cum_pt_mean_lhnfix$mean - off, col="dodgerblue4", lwd=2, lty=1 )
    # lines( df_cum_pt_mean_lhnfix$year, cumsum( df_pt_mean$mean ), col="red", lwd=3, lty=1  )
    ...
    # plot this time series of land c uptake into the previously opened plot
    ## add mean to plot
    lines( dc_cum_merged_sum$year, dc_cum_merged_sum$mean , col="springgreen4", lwd=2 )
    polygon( 
            c( dc_cum_merged_sum$year, rev(dc_cum_merged_sum$year) ), 
            c( (dc_cum_merged_sum$mean - dc_cum_merged_sum$sd ), rev( (dc_cum_merged_sum$mean + dc_cum_merged_sum$sd ) ) ), 
  ...
  ##--------------------------------
    ...
    lines( df_cum_remainder$year, df_cum_remainder$mean , col="darkgoldenrod2", lwd=2 )
    polygon( 
            c( df_cum_remainder$year, rev(df_cum_remainder$year) ), 
            c( (df_cum_remainder$mean - df_cum_remainder$sd ), rev( (df_cum_remainder$mean + df_cum_remainder$sd ) ) ), 
  ...
  ##--------------------------------
    ...
    lines( dc_cum_remainder_merged_sum$year, dc_cum_remainder_merged_sum$mean , col="darkgoldenrod2", lwd=2 )
    polygon( 
            c( dc_cum_remainder_merged_sum$year, rev(dc_cum_remainder_merged_sum$year) ), 
            c( (dc_cum_remainder_merged_sum$mean - dc_cum_remainder_merged_sum$sd ), rev( (dc_cum_remainder_merged_sum$mean + dc_cum_remainder_merged_sum$sd) ) ), 
...
pdf( "fig/tseries_peat_holocene.pdf", width=sum(widths),height=sum(heights) )
  ...
  ##--------------------------------
    ...
    lines( df_pt_mean$year, df_pt_mean$mean / dstep, col="dodgerblue4", lwd=2, lty=1 )

    # ## Add Charly's mean to plot
    # lines( df_charly_pt_mean$year, df_charly_pt_mean$mean / dstep, col="blue", lwd=1 )
  ...
  ##--------------------------------
    ...
    polygon( 
            ...
            c( (df_pt_mean_lehigh$mean - df_pt_mean_lehigh$sd), rev( (df_pt_mean_lehigh$mean + df_pt_mean_lehigh$sd) ) ) / dstep, 
            # c( (df_pt_mean_lehigh$q10), rev( (df_pt_mean_lehigh$q90) ) ) / dstep, 
            col=rgb(0,0,0,0.1), border=NA 
            )
    # lines( df_pt_mean_lehigh$year, df_pt_mean_lehigh$mean / dstep, col="black", lwd=2, lty=1 )
    # lines( df_pt_mean_lehigh$year, df_pt_mean_lehigh$mean / dstep, col="grey50", lwd=1, lty=1 )

    ## fixed after 1200
    polygon( 
            c( df_pt_mean_lhnfix$year, rev(df_pt_mean_lhnfix$year) ), 
            c( (df_pt_mean_lhnfix$mean - df_pt_mean_lhnfix$sd), rev( (df_pt_mean_lhnfix$mean + df_pt_mean_lhnfix$sd) ) ) / dstep, 
            # c( (df_pt_mean_lhnfix$q10), rev( (df_pt_mean_lhnfix$q90) ) ) / dstep, 
            col=add_alpha( "dodgerblue4", 0.3 ), border=NA 
            )
    lines( df_pt_mean_lhnfix$year, df_pt_mean_lhnfix$mean / dstep, col="dodgerblue4", lwd=2, lty=1 )
